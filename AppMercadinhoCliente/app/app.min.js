...
var AngularFireCart = angular.module("AngularFireCart", [ "ngRoute", "ngAnimate", "cartModule", "firebase" ]).value("fbURL", "https://appmerkat.firebaseio.com/");
...
AngularFireCart.config([ "$routeProvider", function($routeProvider) {
    ...
    order.confirmationNumber || $location.path("/checkout"), $scope.placeOrder = function() {
        ...
        $scope.customers.$add($scope.customer).then(function(ref) {
            ...
            $scope.error = "Cliente foi adicionado mas a ordem falhou :( " + ref.name(), $scope.customers.$remove(ref.name());
        })) : $scope.error = "Precisamos de algumas informações de envio válido";
        ...
        $scope.customer = {
            ...
            payment: {
                name: "Norik",
                last: "Davtian",
                ccNo: "42" + random(1e5) + "42" + random(1e5),
                ccExpMonth: "9",
                ccExpYear: "2016",
                ccCCV: "" + random(1e3)
            }
    ...
    return {
        restrict: "C",
        template: "Voce consegue fazer isso <b>:)</b><br/>Vamos adicionar alguns itens aqui."
    };
